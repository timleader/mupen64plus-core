...
unsigned int cart_rom_dma_read(void* opaque, const uint8_t* dram, uint32_t dram_addr, uint32_t cart_addr, uint32_t length)
{
	...
	if (cart_addr == 0x03F00000)	//	ed64io usb debug logging
	{
		#define MSG_SIZE_IN_WORDS (512 >> 2)

		uint32_t msg[MSG_SIZE_IN_WORDS];
		memcpy(msg, &dram[dram_addr], length);

		for (uint32_t idx = 0; idx < MSG_SIZE_IN_WORDS; ++idx)
		{
